...
int parsecommands(char *cmdline, char **argv);
...
*/
void eval(char *cmdline) {
    char buf_commands[MAXLINE];
    char *commands[MAXLINE];
    int pipefds[MAXLINE][2];
    int num_commands = 0;

    char *child_args[MAXARGS];

    char buf_args[MAXLINE];
    char *arguments[MAXARGS];
    int num_args = 0;

    // Split commands by '|' (pipes)
    strcpy(buf_commands, cmdline);
    num_commands = parsecommands(buf_commands, commands);
    ...
    if (num_commands == 0 || (num_commands == 1 && strspn(commands[0], " \t\n") == strlen(commands[0]))) {
        return;
    }

    // Parse arguments of first command 
    strcpy(buf_args, commands[0]);
    num_args = parseline(buf_args, arguments);

    // Check If user has requested a built-in command
    if (!builtin_cmd(arguments)) {

        for (int i = 0; i < num_commands; i++) {
            // Parse command-line arguments
            strcpy(buf_args, commands[i]);
            num_args = parseline(commands[i], arguments);

            // Initialize vars for in/out redirection
            ...
            if (pid == -1) {
                ...
                if (out_redir == 1 && i == num_commands-1) {
                ...
                // If not the last command, set stdout to the next pipe write end
                if (i < num_commands - 1) {
                    close(pipefds[i][0]);
...
int parsecommands(char *cmdline, char **argv) {
    ...
    // Split the input command by pipe '|'
    char *token = strtok(cmdline, "|");
    while (token != NULL && num_args < MAXARGS) {
        ...
        while (len > 0 && (trimmed_command[len - 2] == ' ' || trimmed_command[len - 1] == '\t')) {
            trimmed_command[--len] = '\0';
        }

        // Store the trimmed command
        argv[num_args++] = trimmed_command;
        // Get the next token
        token = strtok(NULL, "|");
    }
    return num_args;
